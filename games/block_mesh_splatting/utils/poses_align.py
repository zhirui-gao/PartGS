...
@torch.no_grad()
def prealign_cameras(pose, pose_GT, device):
...
def transform_points(points, sim3):
    """
    Transform points using a 3D similarity transform.
    """
    # Step 1: Normalize the points by removing translation and scaling from the pose
    points_normalized = (points - sim3.t1) / sim3.s1

    # Step 2: Apply the rotation matrix
    points_rotated = torch.mm(points_normalized, sim3.R.t())

    # Step 3: Apply scaling and translation to map into pose_GT's frame
    points_transformed = points_rotated * sim3.s0 + sim3.t0
