...
def evaluate_mesh(inp, scan_id, dataset_dir, eval_dir, suffix='',step='-1', save_viz=True,num_mesh=1):
    ...
    stl_pcd = o3d.io.read_point_cloud(f'{dataset_dir}/Points/stl/stl{scan_id:03}_total.ply')
    ...
    if save_viz:
        ...
        write_vis_pcd(f'{eval_dir}/vis_{scan_id:03}_d2s{suffix}.ply', data_down, data_color)
        stl_color = np.tile(B, (stl.shape[0], 1))
        stl_alpha = dist_s2d.clip(max=vis_dist) / vis_dist
        stl_color[np.where(above)[0]] = R * stl_alpha + W * (1 - stl_alpha)
        stl_color[np.where(above)[0][dist_s2d[:, 0] >= MAX_DIST]] = G
        write_vis_pcd(f'{eval_dir}/vis_{scan_id:03}_s2d{suffix}.ply', stl, stl_color)
    ...
    with open(f'{eval_dir}/dtu_scores{suffix}_{step}.tsv', 'w') as f:
